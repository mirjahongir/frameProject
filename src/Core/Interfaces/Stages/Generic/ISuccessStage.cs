...
namespace Jh.Core.Interfaces.Stages.Generic
{
    ...
    {
        #region
        //ISuccessStage<T> OnNextAsync(Action action);
        ...
        ISuccessStage<T> OnNextAsync(Task<Action<Result<T>>> method);
        ISuccessStage<T> OnNextAsync<T1>(Action<Task<Result<T>>> method);
        ISuccessStage<T> OnNextq<T1>(Func<Result<T>, T1> method);
